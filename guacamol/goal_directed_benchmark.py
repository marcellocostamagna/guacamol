...
class GoalDirectedBenchmark:
    ...
    def assess_model(self, model: GoalDirectedGenerator, benchmark_number: int) -> GoalDirectedBenchmarkResult:
        """
        ...
        success, result = model.generate_optimized_molecules(scoring_function=self.wrapped_objective,
                                                       requested_num_molecules=number_molecules_to_generate,
                                                       benchmark_number=benchmark_number
                                                       )
        end_time = time.time()

        if not success:
            # raise Exception(f'Error during model assessment: {result}')
            return (False, result)
        else:
            molecules = result
            ...
            return (True, GoalDirectedBenchmarkResult(benchmark_name=self.name,
                                            score=global_score,
